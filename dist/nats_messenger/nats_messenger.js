...
exports.NatsMessenger = void 0;
var _nats = require("nats");
const objToHdrs = msgHeadersObj => {
  const hdrs = (0, _nats.headers)();
  if (msgHeadersObj) {
...
class NatsMessenger {
  ...
  async start() {
    ...
    this.natsConnection = await (0, _nats.connect)(this.config);
  }
  ...
  publish(topic, payload, msgHeaders) {
    ...
    this.natsConnection.publish(topic, sc.encode(payload), {
      headers: hdrs
    });
  }
  ...
  subscribe(topic, callback) {
    ...
    return this.natsConnection.subscribe(topic, {
      callback: (err, msg) => {
        ...
        this.logger.debug(`NatsMessenger.subscribe: callback received err: ${err}, msg: ${receivedPayload}, headers: ${JSON.stringify(receivedHeaders)}`);
  ...
  request(topic, payload, timeout, msgHeaders, reqCb) {
    ...
    return this.natsConnection.request(topic, sc.encode(payload), {
      ...
      this.logger.debug(`NatsMessenger.request.reqCb: msg.data: ${data}, headers: ${JSON.stringify(msgHeaders)}`);
  ...
  response(topic, respCb) {
    ...
    return this.natsConnection.subscribe(topic, {
      callback: async (err, msg) => {
        ...
        this.logger.debug(`NatsMessenger.response.callback: received err: ${err}, msg: '${requestPayload}', headers: ${JSON.stringify(hdrsToObj(msg.headers))}`);
        this.logger.debug(`NatsMessenger.response.respCb: call with err: ${err}, payload: '${requestPayload}', headers: ${JSON.stringify(hdrsToObj(msg.headers))}`);
        const {
          payload = '',
          headers = {}
        } = respCb(err, requestPayload, hdrsToObj(msg.headers));
        this.logger.debug(`NatsMessenger.response.respCb: respond with: payload: '${payload}', headers: ${JSON.stringify(headers)}`);
        const hdrs = objToHdrs(headers);
        await msg.respond(sc.encode(payload), {
          headers: hdrs
        });
      }
    });
...
exports.NatsMessenger = NatsMessenger;